...
export async function getAllNetworkInterfacesService() {
  const networkInterfaces =
    (await si.networkInterfaces()) as si.Systeminformation.NetworkInterfacesData[];

  const networkInterfacesInfo: NetworkInformation[] = networkInterfaces.map(
    (networkInterface: si.Systeminformation.NetworkInterfacesData) => {
      return {
        devices: [
          {
